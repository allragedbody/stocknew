...
def handewritingClassTest():
    ...
    for i in range(mTest):
        ...
        #测试集内容vectorUnderTest，训练集矩阵trainingMat，类型列表hwLabels。
        classifierResult=classify0(vectorUnderTest,trainingMat,hwLabels,3)
        print("分类器返回值为：%d 实际的值为：%d" % (classifierResult,classNumStr))
        if(classifierResult!=classNumStr):
            errorCount+=1.0
    print("错误数为：%d" % errorCount)
    print("错误率为: %f" % (errorCount/float(mTest)))

#handewritingClassTest()

#手写识别算法总结
#1、将测试集中的每一个文件中的内容都转化为1024字符的列表。
#2、拿这个列表针对所有训练集进行训练，通过k临近算法找到最接近的类型。