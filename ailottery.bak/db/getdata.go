...
func (dc *DBClient) GetLotterDataPositive(cur int, size int) ([][]string, error) {
	datas := make([][]string, 0)
	if cur == 0 {
		//查询数据
		rows, err := dc.Conn.Query("select `period`,`number1`,`number2`,`number3`,`number4`,`number5`,`number6`,`number7`,`number8`,`number9`,`number10` from PK10 order by period  limit ?;", size)
		if err != nil {
			return datas, err
		}
		for rows.Next() {
		...
		rows, err := dc.Conn.Query("select `period`,`number1`,`number2`,`number3`,`number4`,`number5`,`number6`,`number7`,`number8`,`number9`,`number10` from PK10 where period <=? order by period  limit ?;", cur, size)
		if err != nil {
			return datas, err
		}
		for rows.Next() {
